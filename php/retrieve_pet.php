<?php
    ...
    if ($execute){
        while ($row = sqlsrv_fetch_object($statement)){
            $arrkeyvalues = array("Name" => $row->name, "Species" => $row->species, "Birthdate" => $row->birthdate, "Weight" => $row->weight, "Street" => $row->street, "City" => $row->city, "State" => $row->state, "Zip" => $row->zip, "Chip" => $row->chip);
        }
