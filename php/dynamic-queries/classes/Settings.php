<?php
    ...
    class SQLSettings {
        ...
        function setTableNames($tables) {
            $index = 0;
            if (is_array($tables)) {
                foreach ($tables as $tableValue) {
                    if ($index !== count($tables) - 1) {
                        if (isset($this->tableNames)) {
                            $this->tableNames .= $tableValue . ', ';
                        } else {
                            $this->tableNames = $tableValue . ', ';
                        }
                    } else {
        ...
        function setSQLKeywords(string $sqlKeyword, string $secondTable, array $sqlKeywordValues) {
            if (is_array($sqlKeywordValues)) {
                switch (strtoupper($sqlKeyword)) {
                    case 'INNERJOIN':
                        $this->sqlKeywords = ' INNER JOIN ';
                        break;
                }
                $this->sqlKeywords .= $secondTable . ' ON ';
                $index = 0;
                foreach($sqlKeywordValues as $keywordKey => $keywordValue) {
                    if ($index !== count($sqlKeywordValues) - 1) {
                        $this->sqlKeywords .= $keywordValue . ' = ';
                    } else {
                        $this->sqlKeywords .= $keywordValue;
                    }
                    $index++;
